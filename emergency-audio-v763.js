/* v76.3 Safari audio final: SpeechSynthesis only, fixed auto-play and short gaps.
   This file intentionally overrides broken/experimental audio handlers. */
(function(){
  'use strict';
  var VERSION='v76.3-safari-audio-final';
  var auto=false;
  var speaking=false;
  ...
  function speakSafari(text,lang){
    ...
    return new Promise(function(resolve){
      ...
      // iPhone Safari sometimes misses onend. Use a short safety timeout so gaps do not become 2–3 seconds.
      var chars = Array.from(text).length;
      var estimated = isJa(u.lang,text) ? (450 + chars * 115) : (420 + chars * 85);
      var maxTimeout = Math.max(900, Math.min(4500, estimated));
      setTimeout(finish, maxTimeout);
    });
  }
  function gapMs(){
    var g=$('gap');
    var v=String((g&&(g.value||g.options&&g.options[g.selectedIndex]&&g.options[g.selectedIndex].text))||'0.4');
    var m=v.match(/[0-9]+(?:\.[0-9]+)?/); var n=m?parseFloat(m[0]):0.4;
    if(!isFinite(n)) n=0.4;
    return Math.max(0, Math.min(3,n))*1000;
  }
  ...
  async function autoLoop(){
    ...
    while(auto){
      ...
      var n=$('nextBtn');
      if(n) n.click();
      else if(window.next) try{ window.next(); }catch(e){}
      await wait(120);
    }
  ...
  function clearBrokenBootError(){
    ...
    if(be && /undefined is not a function|R of T|処理エラー/.test(be.textContent||'')){
      be.textContent='v76.3で音声処理をSafari確定版に切り替えました。まず「音声スタート」→「読む」→「連続再生」を確認してください。';
  ...
  function bind(){
    ...
    var summary=$('voiceDiagSummary'); if(summary && /未診断|Kokoro/.test(summary.textContent||'')) status('v76.3 Safari音声モード','checking');
    ...
    cloneAndBind('openAiFullDiagBtn',function(){ warmAudio(); var el=$('voiceDiagLog'); if(el) el.textContent=''; log('v76.3 Safari音声診断開始'); speakSafari('Safari English voice test.','en-US').then(function(){ return speakSafari('日本語音声テストです。','ja-JP'); }).then(function(){ status('Safari音声診断OK','ok'); log('OK: Safari音声は再生できています。Kokoroは停止中。Safari音声で安定再生します。'); }); },'音声接続診断');
