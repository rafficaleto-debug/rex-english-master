...
(function(){
  'use strict';
  var VERSION='v76-kokoro-core-rebuild';
  ...
  var DEFAULT_SETTINGS={enRate:0.92,jaRate:0.88,voiceEngine:'kokoro',kokoroEnabled:true,kokoroEnglishVoice:'af_heart',kokoroJapaneseVoice:'safari-ja',kokoroRexVoice:'af_heart',kokoroDType:'q8',kokoroDevice:'wasm',openAiEnabled:false,openAiProxyUrl:'',openAiVoice:'af_heart',rexVoice:'af_heart'};
  ...
  function safariSpeak(text,lang){
    return new Promise(function(resolve){
      ...
      u.onend=finish; u.onerror=function(ev){ log('Safari音声エラー: '+(ev&&ev.error?ev.error:'unknown')); finish(); };
      try{ speechSynthesis.speak(u); }catch(e){ log('Safari speak例外: '+(e.message||e)); finish(); }
  ...
  function populateVoiceSelects(){
    ...
    if($('openAiVoiceTestBtn')) $('openAiVoiceTestBtn').textContent='Kokoro英語テスト';
    if($('jaOpenAiTestBtn')) $('jaOpenAiTestBtn').textContent='日本語音声テスト';
    if($('rexOpenAiTestBtn')) $('rexOpenAiTestBtn').textContent='レックスKokoroテスト';
    if($('openAiFullDiagBtn')) $('openAiFullDiagBtn').textContent='Kokoro接続診断';
  }
  ...
  async function diagnose(){
    ...
    await unlock();
    try{ await kokoroSpeak('Hello. This is Kokoro English voice test.','en-US','english'); log('OK: Kokoro英語音声'); }
    catch(e){ log('Kokoro英語NG: '+(e.message||e)); log('Safari予備音声へ切替'); await safariSpeak('Hello. Safari backup voice is working.','en-US'); }
    await safariSpeak('こんにちは。日本語音声テストです。','ja-JP');
    status('診断完了：音声再生ルートを確認しました','ok'); log('OK: 診断完了'); return true;
  }
